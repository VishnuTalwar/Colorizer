...
#
# dataset_url = 'https://www.kaggle.com/datasets/mariomatos/image-colorization'
# od.download(dataset_url)

DATA_DIR = 'unlabeled2017_subsample'
...
# torch.save(net_G.state_dict(), "final_model_weights_2.pt")
...
if __name__ == '__main__':
    net_G = build_res_unet(n_input = 1, n_output = 2, size = 256)
    net_G.load_state_dict(torch.load("colorizer.pt", map_location = device))
    model = MainModel(net_G = net_G)
    model.load_state_dict(torch.load("final_model_weights_2.pt", map_location = device))
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        img_pil = PIL.Image.fromarray(np.uint8(colorized * 255))
        img_pil.save('colorized_image.jpg')
